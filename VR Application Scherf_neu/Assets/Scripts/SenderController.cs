...
public class SenderController : LocalDataSender<TaskData>
{
    ...
    /// </summary>
    private readonly string[] tasks = null; //{ "unplug Device", "remove screw on back of the case", "remove upper panel", "remove screws on top of the case","remove side panels", 
    ...
    {
        if (currentTask != lastTaskSend && tasks != null)
        {
            // send new task
